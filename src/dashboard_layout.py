...
def download_info():
    return dbc.Modal(
            [
                ...
                dbc.ModalBody("Your file is ready for download!"),
            ],
            id="modal",
            fade=True
        )
...
def perfom_download_and_alert_user(data, final_time, download, is_open):
    report = str(Path.home()) + '/' + 'analytics_report' + '/' + 'complete_analytics_report.pdf'
    df=pd.DataFrame.from_records(data)
    final_time=final_time['final_time']
    fig1, fig2= plotting.plot_annotated_heatmaps(df=df, final_time=final_time)
    fig3, fig4 = plotting.plot_cv_analysis(df=df, final_time=final_time)
    ...
    if download:
        ...
        print("Ready to send file for downloading")
        return not is_open, send_file(report)
